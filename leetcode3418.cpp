...
Approach:
1. Use DP with memoization and 3 states (i, j, neutralizations left).
2. At each cell, either:
   - Take the value and move right/down.
   - Skip (neutralize) if value is negative and neutralizations are available.
3. Base case: bottom-right cell.
4. Store results in 3D DP array to avoid recomputation.
5. Return maximum achievable sum.
